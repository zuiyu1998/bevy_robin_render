...
use crate::render_phase::{
    BinnedPhaseItem, BinnedRenderPhaseBatch, BinnedRenderPhaseBatchSet, BinnedRenderPhaseBatchSets,
    CachedRenderPipelinePhaseItem, PhaseItem, PhaseItemBatchSetKey as _, PhaseItemExtraIndex,
    RenderBin, SortedPhaseItem, SortedRenderPhase, UnbatchableBinnedEntityIndices,
...
impl FromWorld for GpuPreprocessingSupport {
    fn from_world(world: &mut World) -> Self {
        ...
        fn is_non_supported_android_device(adapter_info: &RenderAdapterInfo) -> bool {
            bevy_render::get_adreno_model(adapter_info)
                .is_some_and(|model| model != 720 && model <= 730)
                || bevy_render::get_mali_driver_version(adapter_info)
                    .is_some_and(|version| version < 48)
        }
