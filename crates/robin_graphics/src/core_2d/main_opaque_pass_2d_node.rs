use crate::{
    ...
    core_2d::Opaque2d,
    frame_graph::{
        RenderPassBuilder, TransientRenderPassColorAttachment,
...
pub fn main_opaque_pass_2d(
    ...
    let depth_stencil_attachment =
        get_depth_stencil_attachment(depth, &mut render_pass_builder, StoreOp::Store);

    let color_attachment = get_color_attachment(target, &mut render_pass_builder);

    fn get_color_attachment(
        _target: &ViewTarget,
        _render_pass_builder: &mut RenderPassBuilder,
    ) -> TransientRenderPassColorAttachment {
        todo!()
    }

    fn get_depth_stencil_attachment(
        _depth: &ViewDepthTexture,
        _render_pass_builder: &mut RenderPassBuilder,
        _store: StoreOp,
    ) -> TransientRenderPassDepthStencilAttachment {
        todo!()
    ...
    let mut render_pass = TrackedRenderPass::new(&render_device, render_pass_builder);
